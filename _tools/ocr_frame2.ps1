...
function Await($WinRtTask, $ResultType) {
    ...
    catch { Write-Host ("AWAIT FAIL: " + $netTask.Exception.ToString()); throw }
    $netTask.Result
}

try {
  $file = Await ([Windows.Storage.StorageFile]::GetFileFromPathAsync((Resolve-Path $Img).Path)) ([Windows.Storage.StorageFile])
  Write-Host ("file: " + $file.Name + " size=" + $file.Size)
  $stream = Await ($file.OpenAsync([Windows.Storage.FileAccessMode]::Read)) ([Windows.Storage.Streams.IRandomAccessStream])
  Write-Host "stream ok"
  $decoder = Await ([Windows.Graphics.Imaging.BitmapDecoder]::CreateAsync($stream)) ([Windows.Graphics.Imaging.BitmapDecoder])
  Write-Host ("decoder ok " + $decoder.PixelWidth + "x" + $decoder.PixelHeight)
  ...
  Write-Host ("WORDS: " + $result.Lines.Count)
  ...
  Write-Host ("TOP ERR: " + $_.Exception.GetType().FullName + " " + $_.Exception.Message)
